...
MLMInf <-
  function(N,
    ...
    beta_ridge <-
      solve(Sigma_hat + 1 / N * diag(1, p)) %*% crossprod(X, y) / N
    ...
    if (length(active_lasso) > 0) {
      X1 <- X[, active_lasso]
      U <- cbind(X1, as.matrix(Z_block))
      R1 <- quad.tform.inv(crossprod(U), t(y) %*% U)
      R2 <-
        quad.tform.inv(crossprod(U), t(y) %*% U) - quad.tform.inv(crossprod(X1), t(y) %*%
                                                                    X1)
      v_hat <- sqrt((sum(y ^ 2) - R1) / (N - ncol(U)))
      denom <-
        sum(diag(quad.form(
          diag(1, N) - quad.tform.inv(crossprod(X1), X1),
          as.matrix(Z_block)
        )))
      tau_hat <- sqrt((R2 - v_hat ^ 2 * ncol(Z_block)) / denom)
    } else {
      R1 <-
        quad.tform.inv(crossprod(as.matrix(Z_block)), as.matrix(Z_block))
      v_hat <-
        sqrt(1 / (N - sum(diag(R1))) * (as.numeric(crossprod(y)) - as.numeric(quad.form(R1, y))))
      tau_hat <- sqrt((crossprod(y) - N * v_hat ^ 2) / N)
    }
    
    D <- diag(1 / sqrt(v_hat ^ 2 + tau_hat ^ 2 * eZ$values))
    D2 <- diag(sqrt(v_hat ^ 2 + tau_hat ^ 2 * eZ$values))
    ...
    if (length(active_lasso) > 0) {
      ...
      beta_init <- rep(0, p)
      beta_init[active_lasso] <- test$coefficients
    } else {
      beta_init <- rep(0, p)
    }
    ...
    A1 <- quad.tform(D2, LZ)
    A2 <-
      1 / N * tcrossprod(quad.tform(diag(1 / (
        eX$values + lambda_ridge
      )), eX$vectors), X)
    ...
    for (i in 1:sims) {
      ...
      U <- A2 %*% Lepsilon
      Usims[i,] <- U / diag(pX)
      Usims_p[i] <- min(2 * (1 - pnorm(as.vector(abs(
        U
      ) / diagu))))
    }
    ...
    pval <-
      2 * (1 - pnorm(pmax((
        abs(beta_corr / diagu) - Delta / diagu
      ), 0)))
    ...
    return(
      list(
        pval = pval,
        pval_corr = pval_corr,
        l = beta_corr + lower - Delta,
        u = beta_corr + upper + Delta
      )
    )
  }
...
# Assuming independence, Lasso method for linear fixed effect model.
LassoLM <-
  function(N,
           p,
           X,
    ...
    lasso <- glmnet(X, y, family = "gaussian", lambda = best_lambda)
    
    beta_hat <- as.vector(lasso$beta)
    sigma_hat <-
      sqrt(sum((y - X %*% beta_hat) ^ 2) / (N - length(which(beta_hat != 0))))
...
ThetaLambdaUnit <-
  function(N, coord, X, data_select, lambda_list, K) {
    error <- matrix(0, length(lambda_list), K)
    for (j in 1:K) {
      whichj <- data_select == j
      temp <-
        glmnet(X[!whichj,-coord, drop = FALSE], X[!whichj, coord, drop = FALSE], lambda =
                 lambda_list)
      predictions <-
        predict(temp, X[whichj,-coord, drop = FALSE], s = lambda_list)
      error[, j] <- apply((X[whichj, coord] - predictions) ^ 2, 2, mean)
    }
    return(error)
  }

ThetaLambda <- function(N, p, X, lambda_list, K = 10) {
  ...
  err_array  <-
    array(unlist(error), dim = c(length(lambda_list), K, p))
  err_mean   <- apply(err_array, 1, mean)
  err_se     <-
    apply(apply(err_array, c(1, 2), mean), 1, sd) / sqrt(K)
  ...
  lambda_1se <-
    max(lambda_list[err_mean < (min(err_mean) + lambda_min_sd)])
...
MakeThetaHat <-
  function(N,
    ...
    if (type == "min") {
      best_lambda <- ThetaLambda(N, p, X, lambda_list, K = 10)$lambda_min
    }
    if (type == "1se") {
      best_lambda <- ThetaLambda(N, p, X, lambda_list, K = 10)$lambda_1se
    }
    
    for (j in 1:p) {
      temp <-
        glmnet(X[,-j, drop = FALSE], X[, j, drop = FALSE], lambda = best_lambda)
      Beta_hat[-j, j] <- as.vector(temp$beta)
      tau_hat_sq[j] <-
        sum((X[, j] - X[, -j] %*% Beta_hat[j, -j]) ^ 2) / N + best_lambda * sum(abs(Beta_hat[j, -j]))
    }
    diag(Beta_hat) <- 1
