...
generate_data <- function(N, n, m, p, q, X, beta, Z, v, tau, seed){
  set.seed(seed)
  y <- rep(0, N)
  resid <- rep(0, N)
  ...
  for (i in 1:m){ 
    ...
    resid[((i-1)*n+1):(i*n)] <- tcrossprod(Z[((i-1)*n+1):(i*n), ], t(alpha[i,])) + epsilon[((i-1)*n+1):(i*n)]
    y[((i-1)*n+1):(i*n)] <- tcrossprod(X[((i-1)*n+1):(i*n), ], t(beta)) + resid[((i-1)*n+1):(i*n)]
  }
    ...
    for (s_index in 1:length(s_list)){
      for (beta_min_index in 1:length(beta_min_list)){
        for (model_index in 1:length(model_list)){
          ...
          for (i in 1:m){
            Z_block[((i-1)*n+1):(i*n), ((i-1)*q+1):(i*q)] <- Z[((i-1)*n+1):(i*n),]
          }
          ...
          Z <- matrix(0, N, q)
          for (i in 1:m){
            Z[((i-1)*n+1):(i*n),] <- Z_block[((i-1)*n+1):(i*n), ((i-1)*q+1):(i*q)]
          }
          ...
          for (index in 1:trials){
            seed <- seed + 1
            data <- generate_data(N, n, m, p, q, X, beta, Z, v, tau, seed)
            y <- data$y
            run <- tryCatch({mlminf(N, m, n, p, q, X, Z, y, lambda_ridge = 1/N, known = FALSE, sims = 10000, lambda0 = lambda0)}, error = function(err){ return(1) })
            while(length(run) == 1){
              seed <- seed + 1
              data <- generate_data(N, n, m, p, q, X, beta, Z, v, tau, seed)
              y <- data$y
          ...
          filename <- paste(paste("Output/Ridge_lmm", "p", p, "q", q, "model", model, "s", s, "beta.min", beta.min, "fixed_adjust_delta_multi", sep = "_"), ".RData", sep = "")
          save(p, q, model, s, beta_min, test_tracker, tau_tracker, v_tracker, file = filename)
...
#beta <- c(rep(beta.min, s), rep(0, p-s))
...
lmm_tracker <- rep(0, p)
for (index in 1:trials){
  data <- generate_data(N, n, m, p, q, X, beta, Z, v, tau, index)
  y <- data$y
  run <- mlminf(N, m, n, p, q, X, Z, y, lambda_ridge = 1/N, known = FALSE, sims = 10000, lambda0 = lambda0)
...
for (index in 1:trials){
  data <- generate_data(N, n, m, p, q, X, beta, Z, v, tau, index)
  y <- data$y
  run <- lminf(N, m, n, p, X, y, lambda_ridge = 1/N, sims = 10000)
  temp <- (beta >= run$l & beta <= run$u)*1
...
Theta_hat <- make_Theta_hat(N, p, X, lambda_list, K = 10, type = "1se")
for (index in 1:trials){
  data <- generate_data(N, n, m, p, q, X, beta, Z, v, tau, index)
  y <- data$y
  run <- lasso.indep(N, p, X, y, Theta_hat = Theta_hat, sims = 10000, type = "1se")
  temp <- (beta >= run$l & beta <= run$u)*1
  lasso_tracker <- lasso_tracker + temp/trials
}

filename <- paste(paste("Output/Ridge_lmm", "conf_int", "fixed", "ridgedelta", sep = "_"), ".RData", sep = "")
